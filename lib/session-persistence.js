...
module.exports = class SessionPersistence {
  ...
  sortedTodos(todoList) {
    let todos = todoList.todos.map(todo => deepCopy(todo));
  ...
  deleteTodo(todoListId, todoId) {
    ...
    let deletionIndex = todoList.todos.findIndex(todo => todo.id === todoId);
    if (deletionIndex === -1) return false;
    
    todoList.todos.splice(deletionIndex, 1);
    return true;
  }
  ...
  deleteTodoList(todoListId) {
    let deletionIndex = this._todoLists.findIndex(todoList => todoList.id === todoListId);
    if (deletionIndex === -1) return false;

    this._todoLists.splice(deletionIndex, 1);
    return true;
  }
